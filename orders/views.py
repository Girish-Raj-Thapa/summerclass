...
def order_amount(order):
    """Use order._total if you have it; else sum items."""
    if hasattr(order, 'order_total') and order.order_total:
        return int(round(float(order.order_total)))
    return int(sum(op.product.price * op.quantity for op in order.orderproduct_set.all()))


def make_signature(total_amount: int, transaction_uuid: str) -> str:
    """
...
def esewa_start(request, order_id):
    ...
    form = {
        ...
        "signature": make_signature(total_amount, txn_uuid),
    }
...
def esewa_return(request, order_id):
    ...
    """
    order = get_object_or_404(Order, id=order_id, user=request.user)
    encoded = request.GET.get("data") or request.POST.get("data") or \
              request.GET.get("response") or request.POST.get("response")
    ...
    if encoded:
        try:
            ...
            txn_code = payload.get("transaction_code", "")  # e.g. "0000AWE0"
        except Exception:
            pass

    if status == "COMPLETE":
        # This creates Payment if it doesn't exist already
        amount_paid = order_amount(order)
